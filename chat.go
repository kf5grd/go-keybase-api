...
// SendEphemeral sends an exploding chat message
func (c Chat) SendEphemeral(lifetime time.Duration, message ...string) (ChatAPI, error) {
	m := ChatAPI{
		Params: &params{},
	}
	m.Params.Options = options{
	...
	m.Params.Options.Channel = &c.Channel
	m.Params.Options.Message.Body = strings.Join(message, " ")
	m.Params.Options.ExplodingLifetime = lifetime
